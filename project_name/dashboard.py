...
class ProjectDashboard(Dashboard):
    def init_with_context(self, context):
        ...
        self.children.append(modules.AppList(title="Приложения", collapsible=True, column=1, exclude=management_models))
                ...
                children=[
                    {
                        ...
                        "url": reverse_lazy("filebrowser:fb_browse", current_app="filebrowser"),
                    }
                ],
            )
        )

        self.children.append(modules.RecentActions("Последние действия", limit=5, collapsible=False, column=2))

    def get_site_part(self):
        children = [{"title": "Django RQ", "url": reverse_lazy("rq_home")}]
        return modules.LinkList(title="Cайт", collapsible=False, column=2, children=children)
